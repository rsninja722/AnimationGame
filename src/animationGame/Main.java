	...
	public void absoluteDraw() {
		...
		case titleScreen:
			...
			if(introPlaying) {
				...
				// ani
				if(frameCount < 50) {
					Draw.image(Sprites.get("ani"), gw/2, 100, 0, 1.0 + (50-frameCount)/10.0);
				} else {
					Draw.image(Sprites.get("ani"), gw/2, 100);
				}
				
				// mation
				if(frameCount > 50) {
					if(frameCount < 100) {
						Draw.image(Sprites.get("mation"), gw/2, 100, 0, 1.0 + (100-frameCount)/10.0);
					} else {
						Draw.image(Sprites.get("mation"), gw/2, 100);
					}
				}
				...
				if(frameCount > 100) {
					if(frameCount < 150) {
						Draw.image(Sprites.get("game"), gw/2, 100, 0, 1.0 + (150-frameCount)/10.0);
					} else {
						Draw.image(Sprites.get("game"), gw/2, 100);
					}
				}
			}
